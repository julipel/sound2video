...
raw_dir = ROOT / 'outputs' / 'project' / 'audio_raw'
...
for sub in subs:
    ...
    if tempo == 1.0 and fits:
        ...
        filt = None  # natural pace, still overflows -> pass through untouched
    elif fits:
        filt = f'rubberband=tempo={tempo:.4f},apad'
    else:
        filt = f'rubberband=tempo={tempo:.4f}'  # capped speed-up, still overflows
...
if still_over:
    print(f'{len(still_over)} lines still overflow their slot even at {MAX_TEMPO}x '
          f'(total excess {sum(x[1] for x in still_over):.1f}s) -- assemble_output.py will '
          f'push the following line(s) start later to avoid overlapping voices.', flush=True)
