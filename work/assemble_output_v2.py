...
silence_dir = ROOT / 'work' / '_silence_v2'
...
if delayed:
    print(f'{len(delayed)} line(s) pushed later than their speech start (max delay '
          f'{max(d for _, d in delayed):.2f}s): {delayed}', flush=True)

list_path = ROOT / 'work' / '_concat_list_v2.txt'
with list_path.open('w', encoding='utf-8') as f:
    for p in pieces:
        f.write(f"file '{p.resolve().as_posix()}'\n")

track_path = out_dir / 'narration_v2.wav'
...
track_duration = probe_duration(track_path)
print(f'video_duration={video_duration:.3f}  track_duration={track_duration:.3f}', flush=True)

final_path = out_dir / (video.stem + '_ru_translated_dub.mp4')
